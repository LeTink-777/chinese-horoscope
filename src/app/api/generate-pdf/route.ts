 ...
 *
 * Прогноз и есть товар, поэтому запрос обязан подтвердить, что относится к
 * состоявшейся оплате: браузер присылает идентификатор платежа, полученный от
 * /api/checkout, а имя и год рождения читаются из metadata этого платежа, а не
 * из тела запроса. Без проверки эндпоинт отдавал бы платный прогноз любому,
 * кто пришлёт год рождения, а доверие к телу запроса позволило бы покупателю
...
export async function POST(request: Request) {
  ...
  return new NextResponse(new Uint8Array(pdfBuffer), {
    headers: {
      ...
      'Content-Disposition': 'attachment; filename="kitayskiy-goroskop.pdf"',
